...
const SLASH_COMMANDS: { name: string; desc: string }[] = [
  ...
  { name: 'skills', desc: 'loaded skills [all=installed]' },
  { name: 'skill', desc: 'read a skill <name>' },
  { name: 'reload-skills', desc: 're-run the skill auto-router' },
  { name: 'no-auto-skill', desc: 'disable auto-loaded skills' },
  { name: 'unload-skill', desc: 'remove a skill <name> from context' },
  ...
  private async command(raw: string): Promise<void> {
    ...
    switch (cmd) {
      ...
      case 'skills': {
        ...
        this.println(`  ${chip('🎯 skills', 'blue')} ${bold('loaded this session')}`)
      ...
      case 'no-auto-skill': {
        ...
        this.println(`  ${okPill()} auto skill loading off for this session (already loaded ones stay) — /reload-skills re-runs manually`)
        return
      }
      ...
      case 'unload-skill': {
        ...
        if (!removed) return this.println(`  ${errPill()} ${red(`"${arg}" is not loaded (auto or manual) in this session`)}`)
        this.println(`  ${okPill()} ${bold(arg)} removed — it drops out of the context on the next message`)
        return
      }
