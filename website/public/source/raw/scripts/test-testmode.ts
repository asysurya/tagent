...
async function main() {
  ...
  console.log('\n2) system prompt — TEST mode persona')
  {
    ...
    ok('workflow present (serve/browser)', prompt.includes('START IT') && prompt.includes('EXERCISE'))
    ok('responsive step present', prompt.includes('VISUAL QA (web)') && prompt.includes('responsiveness'))
