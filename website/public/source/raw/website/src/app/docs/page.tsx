...
tagent web ~/my-project --no-open`}</Code>
        ...
        <ul className="mt-4 space-y-2 text-zinc-300">
          ...
          <li><b>Plan mode</b> — read-only exploration before committing to changes. When something material is unknown, the agent interviews you with an <b>ask form</b> (see <span className="font-mono text-orange-300">ask_user</span> below).</li>
          <li><b>Test mode</b> — <span className="font-mono text-orange-300">tagent test</span> (or <span className="font-mono text-orange-300">/test</span>, or the Test button in the topbar) turns the agent into a QA engineer: it boots the project (auto-detected dev command), clicks through it with a real Chromium — buttons, inputs, forms — and sees the result of every interaction, screenshots mobile/tablet/desktop, runs a deterministic UI audit (horizontal overflow, tap targets, typography, contrast), and writes <code className="rounded bg-zinc-800 px-1 text-xs">TEST-REPORT.md</code> with a pass/warn/fail verdict. Vision models (GPT-4o/5, Claude, Gemini, GLM-4V…) see the screenshots themselves; text models get the DOM audit. Test mode never modifies source files — the report is its only write. One-time setup in your project: <code className="rounded bg-zinc-800 px-1 text-xs">bun add playwright &amp;&amp; bunx playwright install chromium</code>.</li>
          <li><b>Ask forms (ask_user)</b> — when a decision materially changes the work, the agent asks through an interactive form instead of guessing: <b>option</b> (single choice), <b>multi</b> (pick any) and <b>input</b> (free text) fields, up to 6 questions in one call. Option/multi fields always offer <b>+ add your own option</b>, and every form carries an optional notes box under the fields. The TUI renders it as an overlay (↑↓ move · space/enter pick · <span className="font-mono text-orange-300">a</span> add option · tab next field · esc cancel), the GUI shows a dialog, and headless runs get an honest &quot;no interactive user&quot; answer so the model proceeds with stated assumptions.</li>
          <li><b>Z.ai models config</b> — the built-in provider&apos;s model list is a file: edit <code className="rounded bg-zinc-800 px-1 text-xs">~/.tagent/zai-models.json</code> (global) or <code className="rounded bg-zinc-800 px-1 text-xs">&lt;workspace&gt;/.tagent/zai-models.json</code> (per project) to add, relabel or vision-flag models — same ids replace the built-ins, new ids append, <code className="rounded bg-zinc-800 px-1 text-xs">&quot;replace&quot;: true</code> swaps the whole list. No release wait for a new model.</li>
          <li><b>Context window bar</b> — the hint row under the chat input always shows how much of the model&apos;s window the session eats, opencode-style: <code className="rounded bg-zinc-800 px-1 text-xs">12.3k/131.1k [██████░░░░] 9%</code>. Live per turn (provider usage when reported, else a local CJK-aware estimate), color-coded green/yellow/red. Model windows come from the catalog (each zai-models.json entry can carry <code className="rounded bg-zinc-800 px-1 text-xs">contextWindow</code>), provider seeds or heuristics — <code className="rounded bg-zinc-800 px-1 text-xs">TAGENT_CONTEXT_WINDOW</code> overrides.</li>
          <li><b>Deterministic compaction (<span className="font-mono text-orange-300">/compact</span>)</b> — when the bar crosses 80% (configurable) you get a one-key prompt to ringkas memory; <span className="font-mono text-orange-300">/compact [keep-tokens]</span> works any time. Old turns become one structured digest (who asked what, which tools ran on which paths, statuses, decisions) while recent turns stay verbatim — a 100k-token history lands near 10k. 100% local code: <b>no AI call, nothing invented</b> — every digest line is copied from the transcript, @attachment bodies drop out (files are on disk), and re-compaction folds the prior digest in. Compaction itself costs zero tokens.</li>
          <li><b>Real TUI libraries (v0.18.0)</b> — the terminal interface is built on string-width · figures · cli-boxes · picocolors · wrap-ansi: the boot banner and your messages render as rounded cards (<code className="rounded bg-zinc-800 px-1 text-xs">╭─ ❯ you ─╮</code>), tool lines carry emoji icons with a fixed tool-name column, markdown tables draw as full box tables, and every width counts CJK, emoji and combining marks correctly — columns stay aligned in any language.</li>
