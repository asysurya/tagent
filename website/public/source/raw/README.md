...
| 🤖 **Subagents** | `task` tool spawns isolated agents with their own budget — `general` (full toolset like yours), read-only `explore`, `test` (QA: serve + browser + vision), or custom specialists; subs run in your workspace (they read files themselves) and can never spawn further subs. **Background mode**: `task {"background": true}` returns an id (a1, a2…) instantly — you keep working while the sub runs, its report arrives as a `[SUBAGENT REPORT]` message (auto-resume if you already finished, never asks the user); live view with the `subs` tool, `/subs` for you, parallel limit `subagents.maxParallel` |
...
| 🛟 **Provider fallback** | THREE failover lanes, one per model role — main · subagent · vision — each an ordered multi-key chain (stack the same provider under different keys) with automatic mid-run failover (`/fallback`, `/fallback subagent add …`, Settings in the GUI) |
...
| 🧩 **MCP servers** | Any stdio [Model Context Protocol](https://modelcontextprotocol.io) server — Context7, filesystem, memory, sequential-thinking or your own. Tools appear as `mcp_<server>_<tool>`, same permission gates (`/mcp` in the TUI, Settings → MCP in the GUI). Failures say why in plain language — the real error plus one actionable hint per known cause (disk full, broken npx cache, network), and a missing npx falls back to bunx automatically (v0.22.3) |
...
| 🖥️ **App TUI** | Claude Code-style inline terminal app: the transcript lives in your terminal's own scrollback (scroll with mouse wheel / touch / shift+pgup), a rounded editor box + hint row redraw at the bottom, slash-command palette with tab completion, `@file` mentions, `?` shortcuts, `ctrl+x` quick-action menu (`--classic` for the readline TUI). Tool calls ride colored boxes with bg-chip titles (`╭─[💻 bash]` — bash tomato · MCP red · reads blue…), results & verdicts ride status pills (`✔ done` green / `✗ error` red), every report (MCP · sync · skills · memory · checkpoints) opens with a themed header chip, the reply renders markdown live while streaming, menus pick with `1-9` or a tap, and `/theme` recolors everything — dark · light · tokyo-night · dracula · nord · gruvbox (v0.23.1) |
...
| 🧭 **Operating prompt** | The agent runs the full loop — PLAN → BUILD → TEST → fix → wrap — mirrors YOUR language in every reply, escalates with evidence instead of blind retries, and closes with a structured summary: what was done, files touched, verification results, notes (v0.27.1) |
...
| 🔁 **Mid-run mode switch** | The agent can flip its own mode (`switch_mode` build ↔ plan ↔ test) when the work calls for it — plan approved → build, implementation done → test to verify, test found bugs → build to fix. Every switch needs YOUR approval: the permission card shows the target mode + the agent's reason; on approval the persona and toolset rebuild mid-run (v0.27.1) |
